...
Handoffs recorded on the AgentRun rows (parent_run_id + handoffs list) so the
multi-agent trace is observable, not faked.
"""
import time
import uuid
...
async def run_daily_pipeline(user_id: str, planner_input: dict, db: Session) -> dict:
    """Planner -> (QuestionGen per problem task) with linked audit rows."""
    trace_id = uuid.uuid4().hex[:16]
    handoffs: list[dict] = []
    planner = PlannerAgent()
    pres = await planner.run(planner_input, user_id=user_id, workflow="daily", db=db)
    plan = pres["output"]
    questions = []
    ...
    for t in plan.get("tasks", []):
        if t.get("task_type") in ("problem", "timed_challenge") and t.get("node_id"):
            q = await qgen.run({"pattern": t.get("node_id", "general"),
                                ...
                                "difficulty": "medium"}, user_id=user_id, workflow="daily", db=db)
    ...
    try:
        ...
        db.rollback()
    return {"plan": plan, "questions": questions, "trace_id": trace_id, "handoffs": handoffs}
