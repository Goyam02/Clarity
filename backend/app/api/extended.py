...
async def post_event(session_id: str, body: InterviewEventIn,
    ...
    if body.event_type == "DEBRIEF_COMPLETED":
        raise ClarityError("VALIDATION_ERROR", "This event is generated by the server", 400)
    if session.status == "completed":
        raise ClarityError("SESSION_COMPLETED", "This interview has already ended", 409)
    event_key = body.payload.get("client_event_id")
    if isinstance(event_key, str) and event_key:
        existing = db.query(InterviewEvent).filter(
            InterviewEvent.session_id == session_id,
            InterviewEvent.payload["client_event_id"].as_string() == event_key).first()
        if existing:
            return {"recorded": True, "interviewer": None}
    db.add(InterviewEvent(session_id=session_id, event_type=body.event_type,
                          payload=body.payload))
    db.commit()
    interviewer_say = None
    if body.response_mode == "generate" and body.event_type in ("CANDIDATE_SPEECH", "HINT_REQUESTED", "CODE_CHANGED"):
