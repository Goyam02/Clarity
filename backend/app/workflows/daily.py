...
from app.agents.planner import plan_deterministic, PlannerAgent
...
async def build_daily_plan(db: Session, user_id: str, mood: str, time_available: int,
    ...
    if foundry.mode == "mock":
        plan = plan_deterministic(inp).model_dump()
        trace_id = "mock"
    else:
        res = await PlannerAgent().run(inp.model_dump(), user_id=user_id, workflow="daily", db=db)
