...
| `/onboarding` | 5-screen "Maximum Real Signal" flow | §3 |
...
| `/code-red/interview` | Voice-first AI interviewer (Gemini Live voice-to-voice) + editor reveal | §6b |
...
## Onboarding (spec §3, 5 screens — screenshot-scan step removed)

Account (handled by /signup) → Resume upload (vision) → Real platform pulls
(Codeforces + GitHub public APIs + **LeetCode profile URL and optional
`LEETCODE_SESSION`/`csrftoken` cookies**, all via `/onboarding/signals`) →
...
signal can be skipped without blocking. LeetCode cookies are Fernet-encrypted
at rest and wipeable in Settings; drafts persist under
`clarity_onboarding_draft_v2`.
...
- `src/lib/api/endpoints.ts` — typed wrappers for auth, onboarding/calibration,
  users/settings (incl. LeetCode connect/refresh/disconnect + platforms sync),
  daily plan, mastery graph, CODE RED (create/get/toggle/clear-score/mock-oa),
  companies (CSV+web corpus, drift), interviews (events/transcript/debrief),
  outcomes.
- `src/lib/geminiLive.ts` — **Gemini Live voice-to-voice interviewer**
  (frontend-only, no SFU): `ai.live.connect` with mic PCM streaming (16kHz in,
  24kHz out), input/output transcriptions, `show_editor`/`send_hint` function
  calls, and best-effort transcript mirroring to `/interviews/{id}/events` so
  the backend debrief pipeline is unchanged. Key: `VITE_GEMINI_API_KEY` in
...
frontend/src/
  ...
  onboarding/       5-screen wizard, steps/, components/, data/
  hooks/            useDashboard, usePlatformPulse (fetch + focus refetch + error/retry)
  lib/
    api/            client.ts (transport) + endpoints.ts (typed surface)
    geminiLive.ts   Gemini Live voice interviewer (voice-to-voice + tools)
    ...
    platforms/      daily platform sync (LeetCode + Codeforces) client
